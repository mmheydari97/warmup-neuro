...
n_input = 10
n_output = 4
weights = [1/n_output] * n_output
nn = nl.net.newlvq(nl.tool.minmax(data), n_input, weights)
...
# Define X-Y grids for all 4 classes
...
plt.plot(class_2[:, 0], class_2[:, 1], 'ko')
plt.plot(class_3[:, 0], class_3[:, 1], 'ko')
plt.plot(class_4[:, 0], class_4[:, 1], 'ko')

plt.plot(grid_1[:, 0], grid_1[:, 1], 'm.')
plt.plot(grid_2[:, 0], grid_2[:, 1], 'bx')
plt.plot(grid_3[:, 0], grid_3[:, 1], 'c^')
plt.plot(grid_4[:, 0], grid_4[:, 1], 'y+')
